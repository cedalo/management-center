...
const init = async (client, dispatch, connectionConfiguration) => {
    ...
    for (let i = 0; i < brokerConnections.length; i++) {
        ...
        if (
            (selectedConnectionName && selectedConnectionName === connectionName) ||
            (!selectedConnectionName && connection.status.connected) ||
                ...
                if (currentConnectionName === connection.name) {
                    ...
                    if (!currenlyConnected) {
                        try {
                            ...
                            console.error(
                                `Could not connect to client to the broker ${connection.name}, reason:`,
                                error
                            );
                    ...
                    try {
                        const licenseInformation = await client.getLicenseInformation(
                            licenseInformationTimeoutMilliseconds
                        );
                        dispatch(updateBrokerLicenseInformation(licenseInformation));
                    } catch (error) {
