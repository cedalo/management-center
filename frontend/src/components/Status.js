...
const Status = ({
    ...
    const toNumber = (number) =>
        number === undefined || Number.isNaN(number) ? 'N/A' : new Intl.NumberFormat().format(number);
    ...
    const getBrokerInfos = () => {
        const host = !backendParameters?.urlMappings?.CEDALO_MC_BROKER_CONNECTION_HOST_MAPPING
            ? undefined
            : backendParameters.urlMappings.CEDALO_MC_BROKER_CONNECTION_HOST_MAPPING;
        const mqtt = !backendParameters?.urlMappings?.CEDALO_MC_BROKER_CONNECTION_MQTT_EXISTS_MAPPING
            ? false
            : backendParameters.urlMappings.CEDALO_MC_BROKER_CONNECTION_MQTT_EXISTS_MAPPING === 'mosquitto:true';
        const mqtts = !backendParameters?.urlMappings?.CEDALO_MC_BROKER_CONNECTION_MQTTS_EXISTS_MAPPING
            ? false
            : backendParameters.urlMappings.CEDALO_MC_BROKER_CONNECTION_MQTTS_EXISTS_MAPPING === 'mosquitto:true';
        const ws = !backendParameters?.urlMappings?.CEDALO_MC_BROKER_CONNECTION_WS_EXISTS_MAPPING
            ? false
            : backendParameters.urlMappings.CEDALO_MC_BROKER_CONNECTION_WS_EXISTS_MAPPING === 'mosquitto:true';
        ...
        if (ver1) {
            ...
            infos.push({
                label: 'URL',
                value:
                    currentConnection?.externalEncryptedUrl ||
                    currentConnection?.externalUnencryptedUrl ||
                    currentConnection?.internalUrl ||
    ...
    return (
        <ContentContainer dataTour="page-status" breadCrumbs={<ContainerBreadCrumbs title="Home" />} overFlowX="hidden">
            <ContainerHeader
            ...
            {systemStatus?.$SYS && connected ? (
                    ...
                    <Grid container classes={{ root: classes.container }} spacing={3}>
                        <Grid container item lg={4} xl={4} sm={6} xs={12}>
                            <Grid item xs={12}>
                                <Info
                                    ...
                                    infoTooltip={
                                        <>
                                            The information displayed here is gathered from Mosquitto system topics.{' '}
                                            <br />
                                            Click here to get a detailed explanation.
                                        </>
                                    }
                                    ...
                                    infos={[
                                        ...
                                        },
                                        {
                                            ...
                                            value: toNumber(systemStatus?.$SYS?.broker?.['retained messages']?.count),
                                        },
                                        {
                                            label: 'Bytes Sent',
                        ...
                        <Grid container item lg={4} xl={4} sm={6} xs={12}>
                                ...
                                <Info
                                    ...
                                    actionIcon={
                                        <Tooltip title="Click to inspect clients">
                                            <IconButton className={classes.info} size="small" aria-label="info">
                                                <InspectClientsIcon
                                                    onClick={(event) => {
                                                        event.stopPropagation();
                            ...
                            <Grid item xs={12} style={{ paddingTop: '24px' }}>
                                <Info
                                    label={
                                        brokerLicense && brokerLicense.edition
                                            ? 'Client Usage'
                                            : 'Client Usage not available'
                                    }
                                    infos={[]}
                                    infoIcon
                                    ...
                                    infoTooltip={
                                        <>
                                            The information displayed here is gathered from Mosquitto system topics and
                                            your license. <br />
                                            Click here to get a detailed explanation.
                                        </>
                                    }
                            ...
                            <Grid item xs={12}>
                                {brokerLicenseLoading ? (
                                    ...
                                    <Info
                                        ...
                                        infoTooltip={
                                            <>
                                                The information displayed here is gathered from your license. <br />
                                                Click here to get a detailed explanation.
                                            </>
                                        }
                                        ...
                                        infos={[
                                            {
                                                label: 'Edition',
                                                value:
                                                    brokerLicense.edition === 'pro'
                                                        ? 'Premium'
                                                        : brokerLicense.edition || 'N/A',
                                            },
                                            {
                                                space: true,
                ...
                waitingForSysTopic ? (
                    ...
                    <Alert severity="warning">
                        ...
                        We couldn't retrieve the system status information in the given time window. Please make sure
                        that the user "{defaultClient?.username}" has the rights to read the "$SYS" topic on the
                        selected broker. In rare cases, you may wait a bit longer until system information is finally
                        sent
                    </Alert>
                )
            ) : (
