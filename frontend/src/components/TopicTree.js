...
const TopicTree = ({ topicTree, lastUpdated, currentConnectionName, settings, topicTreeRestFeature, connected }) => {
    ...
    useEffect(() => {
        ...
        if (
            current._message !== selectedNode?._message && // if new message
            current._messagesCounter > selectedNode?._messagesCounter
        ) {
            // quick fix
    ...
    const renderTree = (node, small, medium) => (
        ...
        >
            ...
            {Array.isArray(node.children)
                ? node.children.map((childNode) => renderTree(childNode, small, medium))
                : null}
            {/* </TreeItem> */}
        </StyledTreeItem>
    );
    ...
    return (
        <ContentContainer
            ...
            overFlowX="hidden"
            breadCrumbs={<ContainerBreadCrumbs title="Topic Tree" links={[{ name: 'Home', route: '/home' }]} />}
        >
            <ContainerHeader
                    ...
                    <Grid item xs={8}>
                        <div style={{ display: 'grid', gridTemplateRows: 'max-content auto', height: '100%' }}>
                            ...
                            <div className={classes.treeHeader}>
                                <Typography style={{ fontWeight: '500', fontSize: '0.875rem' }}>Name</Typography>
                                <Typography
                                    style={{
                                ...
                                {selectedNode?._message
                                    ? [
                                          ...
                                          <Paper
                                              variant="outlined"
                                              elevation={1}
                                              style={{ maxHeight: '400px', background: 'none' }}
                                          >
