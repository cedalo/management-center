...
const generateTreeData = (id, name, object) => {
  ...
  if (properties.length > 0) {
    properties.forEach((property) => {
      node.children.push(
        generateTreeData(`${id}/${property}`, property, object[property])
      );
    });
  }
                ...
                <TableBody>
                  ...
                  {selectedNode?._created && (
                    <TableRow>
                      ...
                      <TableCell>
                        {moment(selectedNode?._created).format("LLLL")}
                      </TableCell>
                    </TableRow>
                  )}
                  {selectedNode?._lastModified && (
                    <TableRow>
                      ...
                      <TableCell>
                        {moment(selectedNode?._lastModified).format("LLLL")}
                      </TableCell>
                    </TableRow>
                  )}
                  ...
                  {(selectedNode?._retain === false ||
                    selectedNode?._retain === true) && (
                    <TableRow>
                      <TableCell>
                        <strong>Retain</strong>
                      </TableCell>
                      <TableCell>
                        {selectedNode?._retain ? "yes" : "no"}
                      </TableCell>
                    </TableRow>
                  )}
