...
import { updateUser, updateClusters } from './actions';
...
const createClusterTable = (users, classes, props, onDeleteUser, onUpdateUserRoles, onSelectUser) => {
	const { clusterManagementFeature, userRoles = [], roles = [], onSort, sortBy, sortDirection } = props;

	if (!clusterManagementFeature?.error && clusterManagementFeature?.supported !== false && users && users.length > 0) {
		return <div>
