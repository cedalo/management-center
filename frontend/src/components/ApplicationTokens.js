...
const getDateString = (date, separator = ' ') => {
    // convert to ISO 8601 format date string without seconds portion
    return (
        date.getFullYear() +
        '-' +
...
const getUTCDateString = (date, separator = 'T') => {
    // convert to ISO 8601 format date string without seconds portion
    return (
        date.getUTCFullYear() +
        '-' +
...
const createNewTokenDialog = (dialogOpen, handleDialogClose, client, userRoles, tokens) => {
    ...
    const userInformationMessage =
        'Note that the token will not be saved in the system after generation. Please save it and keep it in a safe place';
    ...
    const createToken = async () => {
        ...
        let createdToken;
        try {
            createdToken = await client.createApplicationToken(
                tokenName,
                role,
    ...
    const closeDialog = async () => {
        ...
        if (isEmptyObject(createdToken) && requiredFieldsChanged) {
            // if one of the requrired fields was changed and we haven't yet generatd token, then we want to preserve state when
...
const createTokenTable = (tokens, classes, props, onDeleteToken) => {
    ...
    if (
        !applicationTokensFeature?.error &&
        applicationTokensFeature?.supported !== false &&
        tokens &&
        tokens.length > 0
    ) {
        return (
                                    ...
                                    >
                                        ...
                                        {small || medium
                                            ...
                                            : [
                                                  ...
                                                  <TableCell>
                                                      {token.requestedBy?.length > 18 ? (
                                                          <Tooltip title={token.requestedBy}>
                                                              <div>{token.requestedBy.substring(0, 18) + '...'}</div>
                                                          </Tooltip>
                                                      ) : (
                                                          <>{token.requestedBy}</>
                                                      )}
                                                  ...
                                                  <TableCell>
                                                      {token.issueDate ? getDateString(new Date(token.issueDate)) : ''}
                                                  </TableCell>,
                                                  <TableCell>
                                                      {token.validUntil
                                                          ? getDateString(new Date(token.validUntil))
                                                          : ''}
                                                  </TableCell>,
                                                  <TableCell>
                                                      {token.lastUsed ? getDateString(new Date(token.lastUsed)) : ''}
                                                  </TableCell>,
                                              ]}
                                        {small ? null : (
                                                        ...
                                                        endAdornment={
                                                            <InputAdornment position="end">
                                                                <IconButton
                                                                    ...
                                                                    onClick={() =>
                                                                        copyText(token.hash, enqueueSnackbar)
                                                                    }
                                                                >
                                                                    <FileCopy fontSize="small" />
                                                                </IconButton>
