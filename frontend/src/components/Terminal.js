...
const Plugins = (props) => {
    ...
    const updateAndDispatchClients = () =>
        brokerClient.listClients().then((clients) => dispatch(updateClients(clients)));

    const updateAndDispatchGroups = () => brokerClient.listGroups().then((groups) => dispatch(updateGroups(groups)));

    const updateAndDispatchRoles = () => brokerClient.listRoles().then((roles) => dispatch(updateRples(roles)));
        ...
        getDefaultACLAccess: (args, print, runCommand) => {
            if (isHelpParameter(args[1])) {
                ...
                brokerClient
                    .getDefaultACLAccess()
                    .then((defaultAccess) => {
                        print(`${defaultAccess.acls.map((acl) => `${acl.acltype}: ${acl.allow}`).join('\n')}`);
                    })
                    .catch((error) => {
                        print(toErrorMessage(error));
                    });
