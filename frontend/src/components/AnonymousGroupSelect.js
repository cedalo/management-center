...
const AnonymousGroupSelect = ({ anonymousGroup, groupsAll = [], onUpdateAnonymousGroup }) => {
    const anonymousGroupDescription = (
        <div>
            ...
            <p>
                This is supported through the automatic anonymous group. If allowed, anything connecting without a
                username will be assigned to a group that you define.
            </p>
            <p>By assigning roles to that group, you can control what anonymous devices can access.</p>
        </div>
    );

    const groupSuggestions = groupsAll.sort().map((groupname) => ({
        label: groupname,
        value: groupname,
    }));
        ...
        >
            ...
            >
                {groupSuggestions.map((group) => (
                    <MenuItem key={group.value} value={group.value} primaryText={group.label}>
                        {group.label}
                    </MenuItem>
                ))}
            </TextField>
            <HtmlTooltip
                title={
                    <React.Fragment>
                        ...
                        <Link
                            href="https://mosquitto.org/documentation/dynamic-security/#:~:text=Anonymous%20access,devices%20can%20access."
                            color="inherit"
                            target="_blank"
                        >
                            <u>{'Read more'}</u>
