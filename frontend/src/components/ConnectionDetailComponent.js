...
const ConnectionDetailComponent = (props) => {
    ...
    const {
        selectedConnectionToEdit: connection = {},
        tlsFeature,
        currentConnectionName,
        alreadyConnected,
        connected: brokerCurrentlyConnected,
    } = props;
    ...
    const [externalUnencryptedUrl, setExternalUnencryptedUrl] = React.useState(
        connection.externalUnencryptedUrl || 'None'
    );
    ...
    const sameConnectionWithClientIDExists = !treatUsernamesAsClientIds
        ? connections?.find((searchConnection) => {
              return (
                  updatedConnection.url &&
                  updatedConnection.credentials?.clientId &&
    ...
    const validate = () => {
        if (
            errors[customCACertificateFieldName] ||
            errors[clientCertificateFieldName] ||
            errors[clientPrivateKeyFieldName]
        ) {
            return false;
        }
        ...
        try {
            ...
            if (connect) {
                try {
                    ...
                    enqueueSnackbar(
                        `Error creating connection "${connection.name}". Reason: ${error.message || error}`,
                        {
                            variant: 'error',
                        }
                    );
    ...
    const checkAndSetErrors = () => {
        ...
        if (updatedConnection[clientPrivateKeyFieldName] && !updatedConnection[clientCertificateFileFieldName]) {
            ...
            (updatedConnection[clientCertificateFieldName] && updatedConnection[clientPrivateKeyFileFieldName]) ||
            (!updatedConnection[clientCertificateFieldName] && !updatedConnection[clientPrivateKeyFileFieldName])
        ) {
            setErrors((prevState) => ({
                ...prevState,
                [clientPrivateKeyFieldName]: null,
                [clientCertificateFieldName]: null,
            }));
        }
    };
        ...
        <FormGroup>
            ...
            {!treatUsernamesAsClientIds && (
                <TextField
                    ...
                    helperText={
                        sameConnectionWithClientIDExists &&
                        'A connection to the same broker with the same client ID already exists.'
                    }
                    id="client-id"
                    label="ClientID"
            ...
            <FormGroup style={{ padding: '5px', paddingLeft: '8px' }}>
                <FormControlLabel
                    control={
                        <Switch
                            color="primary"
                            size="small"
                            disabled={!editMode}
                            checked={updatedConnection.noMetricsMode}
                            onChange={(event) => {
            ...
            <div className={!tlsFeature?.supported ? classes.notEnabledBlock : ''}>
                ...
                <div className={`${classes.parent} ${classes.padTop}`}>
                    <div className={classes.overlayed}>
                        <Typography className={classes.smallFont}>Server certificate</Typography>
                    </div>
                    <Grid
                        container
                        direction={'row'}
                    ...
                    >
                        ...
                        <Grid item xl={6} md={6} sm={6} xs={6}></Grid>
                        ...
                        <Grid item xl={7} md={7} sm={7} xs={7}>
                            <FormGroup row>
                                ...
                                <TextField
                                    ...
                                    className={
                                        errors[customCACertificateFieldName]
                                            ? classes.filenameFieldExpanded
                                            : classes.filenameField
                                    }
                                    size="small"
                                    ...
                                    InputProps={{
                                        endAdornment: (
                                            <IconButton
                                                className={
                                                    editMode && updatedConnection[customCACertificateFileFieldName]
                                                        ? classes.crossButton
                                                        : classes.invisible
                                                }
                                ...
                                <IconButton
                                    disabled={
                                        !tlsFeature?.supported || !updatedConnection[customCACertificateFieldName]
                                    }
                        ...
                        <Grid item xl={2} md={2} sm={1} xs={1}></Grid>
                    </Grid>
                </div>
                <div style={{ padding: '7px' }}></div>
                <div className={`${classes.parent} ${classes.padTop}`}>
                    <div className={classes.overlayed}>
                        <Typography className={classes.smallFont}>Client certificate</Typography>
                    </div>
                    <Grid
                        container
                        direction={'row'}
                    ...
                    >
                        <Grid item xl={3} md={3} sm={4} xs={4}>
                            <Typography
                                className={errors[clientCertificateFieldName] ? classes.verticallyPad : ''}
                                align="left"
                            >
                                Certificate
                            </Typography>
                                ...
                                <TextField
                                    ...
                                    InputProps={{
                                        endAdornment: (
                                            <IconButton
                                                className={
                                                    editMode && updatedConnection[clientCertificateFileFieldName]
                                                        ? classes.crossButton
                                                        : classes.invisible
                                                }
                                                style={{ backgroundColor: 'transparent' }}
                        ...
                        <Grid item xl={3} md={3} sm={4} xs={4}>
                            <Typography
                                className={errors[clientPrivateKeyFieldName] ? classes.verticallyPad : ''}
                                align="left"
                            >
                                Private Key
                            </Typography>
                                ...
                                <TextField
                                    ...
                                    InputProps={{
                                        endAdornment: (
                                            <IconButton
                                                className={
                                                    editMode && updatedConnection[clientPrivateKeyFileFieldName]
                                                        ? classes.crossButton
                                                        : classes.invisible
                                                }
                                                style={{ backgroundColor: 'transparent' }}
