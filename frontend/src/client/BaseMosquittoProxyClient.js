...
export default class BaseMosquittoProxyClient {
  ...
  async connect({ socketEndpointURL } = {}) {
    ...
    try {
      const ws = await this._connectSocketServer(
  ...
  async updateClientGroups(client, groupnames = []) {
    ...
    const groupsToRemove = clientGroupnames.filter(
      (groupname) => !groupnames.includes(groupname)
    );
    const groupsToAdd = groupnames.filter(
      (groupname) => !clientGroupnames.includes(groupname)
    );
  ...
  async updateClientRoles(client, rolenames = []) {
    ...
    if (client.roles) {
      ...
      const rolesToRemove = clientRolenames.filter(
        (rolename) => !rolenames.includes(rolename)
      );
      const rolesToAdd = rolenames.filter(
        (rolename) => !clientRolenames.includes(rolename)
      );
  ...
  async updateGroupClients(group, usernames = []) {
    ...
    const clientsToRemove = groupusernames.filter(
      (username) => !usernames.includes(username)
    );
    const clientsToAdd = usernames.filter(
      (username) => !groupusernames.includes(username)
    );
  ...
  async updateGroupRoles(group, rolenames = []) {
    ...
    if (group.roles) {
      ...
      const rolesToRemove = groupRolenames.filter(
        (rolename) => !rolenames.includes(rolename)
      );
      const rolesToAdd = rolenames.filter(
        (rolename) => !groupRolenames.includes(rolename)
      );
      for (const roleToRemove of rolesToRemove) {
