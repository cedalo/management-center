...
export const isConnectionAllowed = (userProfile, currentConnectionName, permissionFunction) => {
    ...
    if (!userProfile.connections) {
        // if license not present, connections array is not injected into user object
        return undefined;
    }
...
export const isGroupMember = (userProfile) => {
    return (
        ...
        typeof userProfile.connections === 'object' &&
        Object.keys(userProfile.connections).length !== 0
    );
};

// export const isMemberOfAdminGroups = (userProfile) => {
// 	if (isGroupMember(userProfile)) {
// 		for (const connection of userProfile.connections) {
// 			if (adminOrHigher(connection)) {
// 				return true;
// 			}
// 		}
// 	}
