...
const createClusterTable = (clusters, classes, props, onCheckHealthStatus, onDeleteCluster, onSelectCluster) => {
    ...
    if (
        ...
        clusterManagementFeature?.supported !== false &&
        clusters &&
        clusters.length > 0
    ) {
        return (
                        ...
                        {clusters &&
                            clusters.map((cluster) => (
                                ...
                                >
                                    ...
                                    {small ? null : (
                                        <TableCell align={CLUSTER_TABLE_COLUMNS[1].align}>
                                            {cluster.description}
                                        </TableCell>
                                    )}
                                    <TableCell align={CLUSTER_TABLE_COLUMNS[2].align}>
                                        {getSyncModeLabel(cluster.syncmode)}
                                    </TableCell>
            ...
            try {
                    ...
                    description: (
                        <>
                            <DialogContentText style={{ textAlign: 'left' }}>
                                ...
                                <br />
                                <span>
                                    Note that this is only an MMC-side action and does not ensure that the actual
                ...
                await confirm({
                    ...
                    description: (
                        <>
                            <span>
                                Are you sure you want to delete configuration of the cluster "{clustername}" on the side
                ...
                try {
                    ...
                    enqueueSnackbar(
                        `Error deleting cluster "${clustername}" configuration. Reason: ${error_.message || error_}`,
                        {
                            variant: 'error',
                        }
                    );
