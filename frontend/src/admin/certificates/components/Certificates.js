    ...
    const handleDeleteCert = async (cert) => {
        try {
            ...
            if (disconnectedBrokersCount && disconnectedBrokersCount === usedConnections.length) {
                enqueueSnackbar('Cannot remove certificate because the affected brokers are disconnected', {
                    variant: 'error',
                });
                return;
            ...
            await client
                ...
                .catch((error) => {
                    ...
                    if (error.longError) {
                        console.error(
                            `Deletion of "${cert.name}" certificate failed. Details:`,
                            error.longError || error
                        );
                    }
                });
            await loadCerts();
