                            ...
                            <TableRow>
                                {CLIENTS_TABLE_COLUMNS.map((column) => (
                                        ...
                                        style={{
                                            ...
                                            display:
                                                ...
                                                (column.id === 'clientid' && medium)
                                                    ? column.id === 'disconnect' &&
                                                      (clientControlFeature?.error || !clientControlFeature?.supported)
                                                        ? 'none'
                                                        : undefined
                                                    : 'none',
                                        }}
                                    ...
                                    >
                                        <Tooltip
                                            title={
                                                column.id === 'messagesOut'
                                                    ? "Message in subscriber's queue"
                                                    : column.id === 'pagedMessagesOut'
                                                      ? "Message in subscriber's queue paged to disk"
                                                      : ''
                                            }
                                            placement="top"
                                        >
                            ...
                            {clients &&
                                clients.map((client, index) => (
                                    ...
                                    >
                                        ...
                                        {small ? null : (
                                            <TableCell align={CLIENTS_TABLE_COLUMNS[1].align}>
                                                {client.clientid}
                                            </TableCell>
                                        )}
                                        {small || medium
                                            ? null
                                            : [
                                                  <TableCell align={CLIENTS_TABLE_COLUMNS[2].align}>
                                                      {`${client.protocol} ${client.protocolVersion}`}
                                                  </TableCell>,
                                                  <TableCell align={CLIENTS_TABLE_COLUMNS[3].align}>
                                                      {client.address}
                                                  </TableCell>,
                                                    ...
                                                    <>
                                                        {client.lastConnect
                                                            ? [
                                                                  <span>
                                                                      Last Connected:{' '}
                                                                      {dateToString(
                                                                          unixTimestampToDate(client.lastConnect)
                                                                      )}
                                                                  </span>,
                                                                  <br />,
                                                              ]
                                                        ...
                                                        {client.lastDisconnect ? (
                                                            <span>
                                                                Last Disconnected:{' '}
                                                                {dateToString(
                                                                    unixTimestampToDate(client.lastDisconnect)
                                                                )}
                                                            </span>
                                                        ) : null}
                                                        {!client.hasOwnProperty('lastConnect') &&
                                                        !client.hasOwnProperty('lastDisconnect') ? (
                                                            <span>
                                                                Connection state unknown. Check the license of your
                                                                broker for the inspect clients feature
                                                            </span>
                                                        ) : null}
                                                    </>
                                                }
                                            >
                                                {client.connected ? (
                                                    <EnabledIcon
                                                        ...
                                                        id={`enabled-client-icon-${client.username}-${client.clientid}`}
                                                    />
                                                ) : client.hasOwnProperty('connected') ? (
                                                    <DisabledIcon
                                                        fontSize="small"
                                                        style={{ color: red[500] }}
                                                        id={`disabled-client-icon-${client.username}-${client.clientid}`}
                                                    />
                                                ) : (
                                                    <UnknownStateIcon
                                                        fontSize="small"
                                                        style={{ color: grey[500] }}
                                                        id={`unknown-state-client-icon-${client.username}-${client.clientid}`}
                                                    />
                                                )}
                                            </Tooltip>
                                        </TableCell>
                                        {small ||
                                        medium ||
                                        clientControlFeature?.error ||
                                        !clientControlFeature?.supported ? null : (
...
const Clients = (props) => {
    ...
    React.useEffect(() => {
        ...
        sortedClients.forEach((cl) =>
            cl.queues ? (cl.pagedMessagesOut = cl.queues.pagedMessagesOut) : (cl.pagedMessagesOut = 0)
        );
        sortedClients.forEach((cl) =>
            cl.queues
                ? (cl.totalMessagesOut = cl.queues.pagedMessagesOut + cl.queues.messagesOut)
                : (cl.totalMessagesOut = 0)
        );
    ...
    return (
        ...
        >
            <ContainerHeader
                ...
                brokerFeatureWarning={
                    !inspectFeature?.error && inspectFeature?.supported === false ? 'Client Inspection' : null
                }
                warnings={() => {
                    const alerts = [];
