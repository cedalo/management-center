...
const MOSQUITTO_UI_PROXY_CONFIG_DIR =
  process.env.MOSQUITTO_UI_PROXY_CONFIG_DIR || "../config/config.json";
...
const initConnections = (config) => {
  ...
  if (
    process.env.MOSQUITTO_UI_BROKER_NAME &&
    ...
    if (
      process.env.MOSQUITTO_UI_BROKER_USERNAME &&
...
const loadConfig = () => {
  const config = JSON.parse(
    fs
      .readFileSync(path.resolve(__dirname, MOSQUITTO_UI_PROXY_CONFIG_DIR))
      .toString()
  );
  return config;
};
...
const init = (licenseContainer) => {
  ...
  connections.forEach(async (connection) => {
    ...
    try {
      await brokerClient.connect({
        ...
        connectTimeout:
          process.env.MOSQUITTO_UI_TIMOUT_MOSQUITTO_CONNECT || 5000,
      });
  ...
  app.get("/api/update", async (request, response) => {
    const update = await axios.get(
      "https://api.cedalo.cloud/rest/request/mosquitto-ui/version"
    );
  ...
  server.listen(MOSQUITTO_UI_PROXY_PORT, () => {
    console.log(
      `Mosquitto proxy server started on port ${server.address().port}`
    );
  });
