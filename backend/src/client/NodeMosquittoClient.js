...
const CEDALO_MC_TREAT_USERNAMES_AS_CLIENT_IDS = preprocessBoolEnvVariable(
    process.env.CEDALO_MC_TREAT_USERNAMES_AS_CLIENT_IDS
);

const socketErrors = [
    // defined in mqttjs (client.js)
    'ECONNREFUSED',
    'EADDRINUSE',
    'ECONNRESET',
    'ENOTFOUND',
...
const ATTEMPT_BACKOFF_MS =
    (process.env.CEDALO_MC_MQTT_CONNECT_INITIAL_BACKOFF_INTERVAL_MS &&
        Math.abs(process.env.CEDALO_MC_MQTT_CONNECT_INITIAL_BACKOFF_INTERVAL_MS)) ||
    500;
const MAX_NUMBER_OF_ATTEMPTS =
    (process.env.CEDALO_MC_MQTT_CONNECT_MAX_NUMBER_OF_ATTEMPTS || 10) &&
...
const BACKOFF_RATE =
    (process.env.CEDALO_MC_MQTT_CONNECT_BACKOFF_INCREASE_RATE &&
        Math.abs(parseFloat(process.env.CEDALO_MC_MQTT_CONNECT_BACKOFF_INCREASE_RATE))) ||
    1.5;
const CONNECT_TIMEOUT_MS =
    (process.env.CEDALO_MC_MQTT_CONNECT_TIMEOUT_MS &&
...
module.exports = class NodeMosquittoClient extends BaseMosquittoClient {
    ...
    _createConnectionHandler(url, options, oneshot = false) {
        ...
        brokerClient.on('close', () => {
            ...
            if (!MAX_NUMBER_OF_ATTEMPTS) {
                this.logger.log(
                    `No reconnection attempts scheduled for ${this._brokerIdentifier}. CEDALO_MC_MQTT_CONNECT_MAX_NUMBER_OF_ATTEMPTS is zero (${MAX_NUMBER_OF_ATTEMPTS})`
                );
                console.log(
                    `No reconnection attempts scheduled for ${this._brokerIdentifier}. CEDALO_MC_MQTT_CONNECT_MAX_NUMBER_OF_ATTEMPTS is zero (${MAX_NUMBER_OF_ATTEMPTS})`
                );
            ...
            timeoutID = setTimeout(() => {
                // kill set timeout in connect hanlder!!!
                this.logger.log(
    ...
    async _subscribeToSystemTopics() {
        await Promise.all(
            [...this._topicsToSubscribe].map(async (topicname) => {
                // make a copy of _topicsToSubscribe in case it will be modified as we iterate
                const promise = this.subscribe(topicname)
                    .then(() => {
                    ...
                    .catch((error) => {
                        // TODO: reject if unsuccessful for critical topics
                        console.error(`Error subscriting to ${topicname} topic for ${this._brokerIdentifier}:`, error);
                        this.logger.error(
                            `Error subscriting to ${topicname} topic for ${this._brokerIdentifier}: ${error}`
                        );
                    });
                return promise;
            })
    ...
    async _unsubscribeFromAllTopics() {
        await Promise.all(
            [...this.subscribedTopics].map(async (topicname) => {
                // make a copy of subscribedTopics in case it will be modified as we iterate
                const promise = this.unsubscribe(topicname)
                    .then(() => {
                    ...
                    .catch((error) => {
                        console.error(
                            `Error unsubscribing to ${topicname} topic for ${this._brokerIdentifier}:`,
                            error
                        );
                        this.logger.error(
                            `Error unsubscribing to ${topicname} topic for ${this._brokerIdentifier}: ${error}`
                        );
                    });
                return promise;
            })
    ...
    _connectBroker(url, options, oneshot, successCallback = () => {}, errorCallback = () => {}) {
        ...
        if (!brokerClient) {
            if (!url || !options) {
                throw new Error(
                    'connection handle not initialized and no arguments given, invalid call to _connectBroker'
                );
            }
            try {
    ...
    async _disconnectBroker(isNormalDisconnect) {
        ...
        // this._client?.end();
        await addTimeout(
            closeBrokerConnection(this._client),
            5000,
            `Disconnecting broker ${this._brokerIdentifier} timed out`
        );
