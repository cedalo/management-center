...
const adapter = new FileSync(
    path.join(process.env.CEDALO_MC_DIRECTORY_SETTINGS || getBaseDirectory(__dirname), 'db.json')
);
...
const generateSecret = () => {
    // TODO: change the location of this fucntion, also use it in plugins' secret generator, or transfer secretGenerator code here
...
const replaceNaN = (number, replacer) => {
    if (isNaN(number)) {
        // check for NaN is also possible with (number !== number) will be true only if number is NaN
        return replacer;
    }
    return number;
};
