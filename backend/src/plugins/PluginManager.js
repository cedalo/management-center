...
module.exports = class PluginManager {
    ...
    static loadPluginList() {
        let plugins;
        try {
            const pathToPluginList =
                CEDALO_MC_PLUGIN_LIST_PATH || path.join(getBaseDirectory(__dirname), 'plugins.json');
            plugins = fs.readFileSync(pathToPluginList, 'utf8');
            plugins = JSON.parse(plugins);
        } catch (error) {
            if (CEDALO_MC_PLUGIN_LIST_PATH) {
                // if we explicitely defined where plugin list should be loacated and it failed to load, then we display an error
    ...
    _sortPluginList(plugins) {
        ...
        const PLUGIN_IDS_OF_HIGHEST_PRIORITY = [
            'multiple-connections' /* 'login_rate_limit' */,
            ,
            'saml-sso',
            'ctrlx-integration',
            ...OS_PLUGINS_IDS,
    ...
    _preprocessRequiredPlugin(pluginClass, pluginConfiguration) {
        const enablePlugin =
            pluginConfiguration.enableAtNextStartup !== undefined ? !!pluginConfiguration.enableAtNextStartup : true;
    ...
    _requirePlugins(pluginList) {
        ...
        const load = (pluginConfiguration, plugins) => {
            ...
            if (
                !CEDALO_MC_OFFLINE &&
                (pluginConfiguration.id === 'usage-analytics' || pluginConfiguration.id === 'all')
            ) {
                try {
            ...
            if (pluginConfiguration.id === 'ctrlx-integration' || pluginConfiguration.id === 'all') {
                try {
                    pluginClass =
                        require('../../../../integrations/ctrlx/plugins/ctrlx-integration-authentication').Plugin;
                    const requiredPluginObject = this._preprocessRequiredPlugin(pluginClass, pluginConfiguration);
                    plugins.push(requiredPluginObject);
                } catch (error) {
        ...
        if (!pluginList || !Array.isArray(pluginList)) {
            // if plugin list is null or undefined - everything from license is loaded. if plugin list is an empty list then no plugins are loaded
            load({ id: 'all' }, plugins);
            return plugins;
        }
        ...
        for (const pluginConfiguration of pluginList) {
            if (
                ...
                throw new Error(
                    `Plugin entry "${JSON.stringify(pluginConfiguration)}" in plugin config list is invalid`
                );
            }
    ...
    init(pluginList, context, swaggerDocument) {
        ...
        if (licenseContainer.license.isValid) {
            ...
            allRequiredPlugins.forEach((requiredPlugin) => {
                ...
                const plugin = new Plugin({
                    enableAtNextStartup: shouldEnable,
                    context,
                    hidden: false /*!!pluginConfiguration.hidden*/,
                });
                if (
                    ...
                    if (!shouldEnable) {
                        console.log(
                            `Plugin not loaded: Plugin set to be disabled at current startup: "${plugin.meta.id}"`
                        );
        ...
        for (let i = 0; i < this._plugins.length; i++) {
            ...
            try {
                ...
                if (plugin.swagger) {
                    ...
                    swaggerDocument.components.schemas = Object.assign(
                        swaggerDocument.components.schemas || {},
                        plugin.swagger.components?.schemas
                    );
                    swaggerDocument.components.statuses = Object.assign(
                        swaggerDocument.components.statuses || {},
                        plugin.swagger.components?.statuses
                    );
                }
